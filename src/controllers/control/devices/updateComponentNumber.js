...
exports.updateComponentNumber = async (req, res) => {
  try {
    ...
    // Hash the component number before saving
    device.componentNumber = crypto.createHash('sha256').update(componentNumber.trim()).digest('hex');
    await device.save();

    res.json({ message: 'Component number updated successfully', hashedComponentNumber: device.componentNumber });
  } catch (error) {
